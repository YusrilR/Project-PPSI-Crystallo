...
<body>
                                        ...
                                        <tbody>
                                            <tr>
                                                ...
                                                <td style="text-align: center; width: 15%">
                                                    <button type="button" class="btn btn-primary btn-sm btn-flat btn-xs" data-toggle="modal" id="btn_edit" data-id="<?php echo $mysql['Id_PenggajianPekerja']; ?>" data-target='#updateModal<?php echo $mysql["Id_PenggajianPekerja"] ?>'><i class="fa fa-pencil"></i> Edit</button>
                                                </td>
                                            </tr>
                                        </tbody>
                                    <?php } ?>
                        ...
                        <div class="modal-body">
                            <form action="./function/function_penggajianpekerja.php?act=updatepenggajian" method="post" role="form">
                                <div class="row form-group">
                                    ...
                                    <div class="col-12 col-md-9"><input type="text" id="Id_PenggajianPekerja" name="Id_PenggajianPekerja" placeholder="Text" class="form-control" value="<?php echo $row['Id_PenggajianPekerja']; ?>" readonly></div>
                                </div>
                                <div class="row form-group">
                                    <div class="col col-md-3"><label for="text-input" class=" form-control-label">Tanggal Penggajian</label></div>
                                    <div class="col-12 col-md-9"><input type="date" id="Tgl_PenggajianPekerja" name="Tgl_PenggajianPekerja" placeholder="Deskripsi" class="form-control" value="<?php echo $row['Tgl_PenggajianPekerja']; ?>" readonly></div>
                                </div>
                                <div class="row form-group">
                                    <div class="col col-md-3"><label for="text-input" class=" form-control-label">Nama Pekerja</label></div>
                                    <div class="col-12 col-md-9">
                                        <select name="Id_Pekerja" id="Id_Pekerja_edit" class="form-control" required readonly>
                                            <?php
                                ...
                                <div class="row form-group">
                                    ...
                                    <div class="col col-md-9"><input type="text" id="Id_Gaji_edit" name="Id_Gaji" class="form-control" value="<?php echo $row['Id_Gaji']; ?>" readonly></div>
                                </div>
                                <!-- <div class="row form-group">
                                    <div class="col col-md-3"><label for="text-input" class=" form-control-label">Deskripsi Gaji</label></div>
                                    <div class="col col-md-9"><input type="text" id="Deskripsi_Gaji_edit" name="Deskripsi_Gaji" class="form-control" readonly></div>
                                </div> -->
                                <div class="row form-group">
                                    <div class="col col-md-3"><label for="text-input" class=" form-control-label">Jumlah Produksi</label></div>
                                    <div class="col col-md-9"><input type="text" id="Jumlah_Produksi_edit" name="Jumlah_Produksi_edit" placeholder="Jml" class="form-control" value="<?php echo $row['Jml_Produksi']; ?>" readonly></div>
                                </div>
                                <!-- <div class="row form-group">
                                    <div class="col col-md-3"><label for="text-input" class=" form-control-label">Id Gaji</label></div>
                                    <div class="col-12 col-md-9">
                                        <select name="Id_Gaji" id="Id_Gaji_edit" class="form-control" required >
                                            <option disabled selected>Pilih Gaji</option>
                                            <?php
                                            $queryJenis = mysqli_query($koneksi, "SELECT * FROM `tabel_gaji` WHERE Deskripsi LIKE '%Pekerja%'");
                                            if (mysqli_num_rows($queryJenis)) {
                                                $selected = "";
                                                while ($row2 = mysqli_fetch_assoc($queryJenis)) :

                                            ?>
                                                    <option <?php if ($row['Id_Gaji'] == $row2['Id_Gaji']) echo "selected"; ?> value="<?= $row2['Id_Gaji']; ?>"><?= $row2['Deskripsi']; ?></option>
                                            <?php endwhile;
                                            } ?>
                                        </select>
                                    </div>
                                ...
                                <div class="row form-group">
                                    ...
                                    <div class="col-12 col-md-9">
                                        <div class="input-group">
                                            ...
                                            <input type="text" id="Besar_Gaji_edit" name="Besar_Gaji" placeholder="Besar Gaji" class="form-control" readonly >
                                ...
                                <div class="row form-group">
                                    ...
                                    <div class="col-12 col-md-9">
                                        <div class="input-group">
                                            <div class="input-group-addon"><i class="fa fa-money"></i></div>
                                            <input type="text" id="Grand_Total_edit" name="Grand_Total_edit" placeholder="Grand Total" class="form-control" value="<?php echo $row['Grand_Total']; ?>" readonly>
                                            <div class="input-group-addon">,00</div>
                                        </div>
                                    </div>
                                </div>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            //ketika klik tgl akhir - tambah
            $('#Tgl_Akhir').on('change',function(){
                var id = $('#Id_Pekerja').val()
                var tglakhir = $(this).val()
                var tglawal = $('#Tgl_Awal').val()
                $.ajax({
                    url:'../kepegawaian/function/function_gaji.php',
                    method: 'GET',
                    data: {'act':'getproduksi',
                            'Id_Pekerja': id,
                            'Tgl_Awal': tglawal,
                            'Tgl_Akhir': tglakhir},
                    success: function(data){
                        data = JSON.parse(data)
                        $('#Jumlah_Produksi').val(data['Jumlah_Produksi'])
                        console.log(data);
                    }
                })
            })
            //ketika klik tgl akhir - edit
            $('#Tgl_Akhir_edit').on('change',function(){
                var id = $('#Id_Pekerja').val()
                var tglakhir = $(this).val()
                var tglawal = $('#Tgl_Awal_edit').val()
                $.ajax({
                    url:'../kepegawaian/function/function_gaji.php',
                    method: 'GET',
                    data: {'act':'getproduksiedit',
                            'Id_Pekerja': id,
                            'Tgl_Awal_edit': tglawal,
                            'Tgl_Akhir_edit': tglakhir},
                    success: function(data){
                        // data = JSON.parse(data)
                        // $('#Jumlah_Produksi_edit').val(data['Jumlah_Produksi'])
                        console.log(data);
                    }
                })
            })
            //ketika combobox gaji diganti value - edit
            $('#Id_Gaji_edit').on('change',function(){
                var id = $(this).val()
